		...
		<template v-if="lessonData && isJoinAgency">
		    <!--本节任务-->
				...
				<div class="Lesson-punch" v-if="trialReading === '0'">
			      ...
			      ></lessondynamicItem>
			      <div class="Expand-btn" @click.stop="toPunchList('excellent')" v-if="excellentPunchList.length>=5">查看所有优秀打卡 <span>({{peopleCardInfo.isExcellentCard}})</span></div>
					</div>
				...
				<div class="all-punch" v-if="trialReading === '0' && peopleCourseCardList && peopleCourseCardList.length>0">
		         ...
		         @disableOperationEvents="operation"
		      ></lessondynamicItem>
		      <div class="Expand-btn all-show" @click.stop="toPunchList('all')" v-if="peopleCourseCardList.length>=5">查看所有打卡 <span>({{peopleCardInfo.isCurrentStudy}})</span></div>
				</div>
    ...
    communityId = ""
  	...
  	created(){
  		...
  		Promise.all([lessonContentApi(this.$route.query.id),getCourseCardListApi(parama)]).then((res)=>{
  			...
  			this.peopleCardInfo = res[0].peopleCardInfo
