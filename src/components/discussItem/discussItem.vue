    ...
    <div :class="{right: true, border: !hideBorder}">
      ...
      <div class="content-head">
        <div class="user-box" v-if="item.reviewer.role.isShow">
          <div>
            <!-- 用户名  :class="item.reviewer.role.title === '塔主' || item.reviewer.role.title === '嘉宾' ? 'master' : 'guest'"  <span class="label" v-if="item.reviewer.role.title === '管理员'">{{item.reviewer.role.title}}</span>-->
            <p class="user-name" @click.stop="toUserInfo(item.reviewer.userId)">{{item.reviewer.realName}}</p>
            <!-- 用户头衔 -->
