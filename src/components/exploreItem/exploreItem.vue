<template>
  ...
  <div class="explore-item" :class="{border: !hideBorder}" @click="toDetails">
    ...
    <div class="content-head">
      ...
      <div class="user-box">
        <div>
         <span class="user-name" :class="item.releaseUser.role && item.releaseUser.role.title === '塔主' || item.releaseUser.role.title === '嘉宾' ? 'master' : 'guest'" @click.stop="toUserInfo(item.releaseUser.userId)">{{item.releaseUser.realName}}<span class="administrators" v-if="item.releaseUser.role && item.releaseUser.role.title === '管理员'">管理员</span></span>
